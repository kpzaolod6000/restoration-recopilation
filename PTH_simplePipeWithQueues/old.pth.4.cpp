...
#define BUFSIZE 10//00
#define MAXDATA 50//00
#define NRSTAGES 3
...
void Pipe(void* a1, void* a2, void* a3) {
  ...
  do {
    ...
    if (i_1 >=0) {
      ...
      printf("myOutputQueue_1->nr_elements : %i\n", myOutputQueue_1->nr_elements);
    }

    printf("my_input_2 : %i, my_input_3: %i\n", my_input_2, my_input_3);

    // STAGE TWO
    if (my_input_2>0) {
      my_input_2 = read_from_queue(myInputQueue_2);
      printf("myInputQueue_2->nr_elements : %i (%i)\n", myInputQueue_2->nr_elements, myOutputQueue_1->nr_elements);
      if (my_input_2 > 0)
