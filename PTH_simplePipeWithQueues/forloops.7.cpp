...
void Pipe(void** a0, void* a1, void* a2, void* a3) {
  ...
  for (i1 = MAXDATA ; i1 > 0; i1--) {
    printf("S1 : %d\n",i1);
    my_output1 = i1;
    // myOutputQueue1->elements[myOutputQueue1->addTo] = my_output1; // WRITE
    // myOutputQueue1->addTo = (myOutputQueue1->addTo + 1) % myOutputQueue1->capacity; // UPDATE
    // myOutputQueue1->nr_elements++; // UPDATE
    printf("S1 break\n");

    int elem2;
    // elem2 = myInputQueue2->elements[myInputQueue2->readFrom]; // READ
    elem2 = my_output1;
    // myInputQueue2->nr_elements--; // UPDATE
    // myInputQueue2->readFrom = (myInputQueue2->readFrom + 1) % myInputQueue2->capacity; // UPDATE
    my_input2 = elem2;
    printf("S2 : %d\n",my_input2);
    my_output2 = my_input2 + 1;
    printf("S2.out : %d\n",my_output2);
    // myOutputQueue2->elements[myOutputQueue2->addTo] = my_output2; // WRITE
    // myOutputQueue2->addTo = (myOutputQueue2->addTo + 1) % myOutputQueue2->capacity; // UPDATE
    // myOutputQueue2->nr_elements++; // UPDATE

    int elem3;
    // elem3 = myInputQueue3->elements[myInputQueue3->readFrom]; // READ
    elem3 = my_output2;
    // myInputQueue3->nr_elements--; // UPDATE
    // myInputQueue3->readFrom = (myInputQueue3->readFrom + 1) % myInputQueue3->capacity; // UPDATE
    my_input3 = elem3;
    printf("S3 : %d\n",my_input3);
