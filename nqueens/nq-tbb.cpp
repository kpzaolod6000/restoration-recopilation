...
void PipeTBB(int board_size, int depth) 
{
    tbb::parallel_pipeline( /*max_number_of_live_token=*/ 2,
        tbb::make_filter<void,task_t>(
            tbb::filter_mode::serial_in_order,
            [&](tbb::flow_control& fc)-> task_t{
                task_t in = workerStage1(board_size, depth);
                if (in.last)
                    fc.stop();                     
                return in;
            }    
        ) &
        tbb::make_filter<task_t,int>(
            tbb::filter_mode::parallel,
            [](task_t task){
              int cnt = workerStage2(task);
              return cnt;
            }
        ) &
