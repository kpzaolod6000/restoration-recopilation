...
void *farm_worker(void *arg)
{
    task_t t, r;
    while(1)
    {
         t = read_from_queue(&tq);
...
int main(int argc, 
   ...
   // farm bit
   InitialiseQueue(&tq, MAXTASKS);
   InitialiseQueue(&tq2, MAXTASKS);
  ...
  pthread_attr_t attr;  
    ...
    for (i=0; i<nworkers; i++) {
        pthread_create(&workers[i], &attr, farm_worker, NULL);
    }
   
   //  int i;
   for (i=0;i<streamlen;i++) {
     //      fprintf(stderr, "Getting Task in main %d\n", i);
        add_to_queue(&tq, tasks[i]);
