    ...
    while(1)
    {
        ...
        if ((int)(size_t)t1 == 42) {
            ...
            do {
                ...
                if (tq2->addat == tq2->removeat-1) 
                {   tq2->lock = 0 ;
                    unlock_t_task_t++;
                }

            } while (tq2->addat == tq2->removeat-1);

            tq2->ts[tq2->addat++] = (task_t2 *)42;
            if (tq2->addat == MAX_TASKS)
                tq2->addat = 0;

                tq2->lock = 0 ;
                unlock_t_task_t++;

            if ((tq2->removeat+1) % MAX_TASKS == tq2->addat) {
            }
       ...
       // gettask
        do {
            ...
            if (tq2->removeat == tq2->addat) {
                tq2->lock = 0 ;
                unlock_t_task_t++;
            }
        }  while (tq2->removeat == tq2->addat);

        t2 = tq2->ts[tq2->removeat];

        if (++tq2->removeat == MAX_TASKS) {
            tq2->removeat = 0;
        }
        tq2->lock = 0 ;
        unlock_t_task_t++;
...
int main(int argc, char* argv[])
{
  ...
  for (int i=0; i<NIMGS; i++) {
    ...
    sprintf(images[i],"/Users/chrisb/Documents/Papers/hlpp2020/restoration/fasterflow/src/chris/convolution/examples/Simple_Convolution/Heterogeneous_FF/images/image%d.png", i);
