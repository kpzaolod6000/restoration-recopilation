...
task_t2 *workerStage1(int i)
{
    ...
    char* images = (char *) malloc (sizeof(char)*1000);
    
    sprintf(images,"/home/judal/Documentos/Paralela/restoration-master/convolution/v2/image%d.png",i);

    // for (size_t i_ = 0; i_ < 100; i_++)
    // {
    //   cout << images[i_]<<" ";
    // }
...
void RunPipeline(int ntoken, int NIMGS) {
    tbb::parallel_pipeline(
        ...
        tbb::make_filter<void,task_t2*>(
            tbb::detail::d1::filter_mode::serial_in_order, Stage1(NIMGS) )
            // tbb::filter::serial, Stage1(NIMGS) )
    &
        tbb::make_filter<task_t2*,int>(
            tbb::detail::d1::filter_mode::parallel, Stage2() )
            // tbb::filter::parallel, Stage2() )
    &
        tbb::make_filter<int,void>(
            tbb::detail::d1::filter_mode::serial_in_order, Stage3() ));
